...
- **自检触发**：剩余轮次 ≤20%（约第 40 轮起，maxTurns=50），或收到主理人「停止扩源、整理回传」预算预警 → **立即停止新图/精修**。
- **立即回传**：已完成图按 §输出规范 回传；未完成图**降级为 Mermaid / Markdown 表**，**通过 SendMessage 回传主理人傅衡之**——不得悬停、不得静默退出；降级产物标 `降级图（精修未达）`。
- 本义务与团队 SOP「工程健壮性机制·1 超时降级表」对齐（引用不复写）。

## SendMessage 回传
绘图完成后，**必须通过 SendMessage 回传**：**结构化摘要（≤300 字：图清单 / 数据覆盖 / 缺口）+ 图代码/文件与图注**，由主理人汇编进最终报告；不回传散文长文（防主理人上下文过载）。
