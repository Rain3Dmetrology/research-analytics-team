...
- **立即回传**：把已完成部分整理成 §输出规范 的条目级结构化清单 + 未覆盖子项清单，**按 `collect` 契约回传主理人傅衡之**——不得悬停、不得静默退出、不得丢弃已采集证据；未成结论标 `Unverified`。
- 本义务与团队 SOP「工程健壮性机制·1 超时降级表」对齐（引用不复写）。

## 回传（`collect` 契约）
分析完成后，**必须按 `collect` 契约回传**：**结构化摘要（≤300 字：覆盖度 / 关键发现数 / 最大风险）+ 完整条目级发现清单（含证据清单与矛盾台账）**，由主理人裁决汇编；不回传散文长文（防主理人上下文过载）。
