...
- **立即回传**：把已完成测算表整理成 §输出规范 结构 + 未完成维度清单（如 SOM 缺 bottom-up），**按 `collect` 契约回传主理人傅衡之**——不得悬停、不得静默退出、不得丢弃已算结果；缺失维度标 `Single-source / 待补`。
- 本义务与团队 SOP「工程健壮性机制·1 超时降级表」对齐（引用不复写）。

## 回传（`collect` 契约）
分析完成后，**必须按 `collect` 契约回传**：**结构化摘要（≤300 字：覆盖度 / 关键测算结论 / 最大口径风险）+ 完整数据测算表与口径说明**，由主理人裁决汇编；不回传散文长文（防主理人上下文过载）。
